...
public class MapCells : MonoBehaviour
{
    ...
    private HashSet<GameObject>[,] pathBlockerData;        // 每个网格登记的挡路对象集合（建筑和兵种）。
    ...
    /// blocksPath 为 true 时同时写入挡路账，兵种和建筑都登记挡路。
    /// </summary>
    /// <param name="cells">需要登记占用的网格坐标。</param>
    /// <param name="occupier">占用这些网格的游戏对象。</param>
    /// <param name="blocksPath">该对象是否阻挡寻路。</param>
    ...
    /// <summary>
    /// 判断单个有效网格是否被挡路对象占用，可忽略执行者自身和当前攻击目标。
    /// 供 A* 寻路判障使用，越界或未初始化时返回 false。
    /// </summary>
    /// <param name="cell">需要检查的网格坐标。</param>
    /// <returns>有效网格被挡路对象占用时返回 <see langword="true"/>。</returns>
    public bool IsPathBlocked(Vector2Int cell, GameObject self = null, GameObject target = null)
    {
        if (pathBlockerData == null || !IsInRange(cell.x, cell.y)) return false;
        foreach (GameObject occupier in pathBlockerData[cell.x, cell.y])
        {
            if (occupier != null && occupier != self && occupier != target)
                return true;
        }
        return false;
    }
